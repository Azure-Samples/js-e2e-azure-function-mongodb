...
curl -X POST http://YOUR-FUNCTION-RESOURCE-NAME/api/category?code=YOUR-FUNCTION-KEY \
   -H 'Content-Type: application/json' \
   -d '{"document":{"name":"Sally"}}' --verbose

curl -X GET http://YOUR-FUNCTION-RESOURCE-NAME/api/category?code=YOUR-FUNCTION-KEY \
   -H 'Content-Type: application/json' --verbose
...
curl -X DELETE http://YOUR-FUNCTION-RESOURCE-NAME/api/category?code=YOUR-FUNCTION-KEY \
   -H 'Content-Type: application/json' \
   -d '{"id":"DOCUMENT_ID"}' --verbose
